...
| Задача | Статус |
|---|---|
| Перенести утверждённую зачётку и строгий список уроков | Сделано |
| Оставить в lesson row только `ошибки N` или `без ошибок` | Сделано |
| Добавить вкладки `УРОКИ` / `ОШИБКИ N` на реальных `failedCardKeys` | Сделано |
| Сформировать общий card-level focus scope по ошибочным урокам | Сохранено через существующий callback |
| Подключить CTA и empty/disabled states без изменения in-progress курса и Speaker | Сделано |
| Собрать compile-safe commit и push | В работе |

# Final pass — one diagnostic lesson metric

| Решение | Статус |
|---|---|
| Убрать `4/4 выучено` из каждой строки как дублирующую метрику | Зафиксировано |
| Убрать индивидуальный процент закрепления из lesson list | Зафиксировано |
| Показывать только `ошибки 2` при наличии ошибок | Зафиксировано |
| Для чистого урока показывать `без ошибок` нейтральным текстом | Зафиксировано |
| Обновить финальный mockup без перегруза | Не начато |

# Final pass — native lesson metrics

| Решение | Статус |
|---|---|
| Убрать слово `Фокус` из lesson rows | Зафиксировано |
| Показывать `4/4` как главный прогресс строки | Зафиксировано |
| Показывать `ошибки 2` только при наличии ошибок | Зафиксировано |
| Показывать `закрепление 67%` как спокойное вторичное поле | Зафиксировано |
| Сгенерировать финальный mockup без перегруженных inline labels | Не начато |

# Redesign reset — editorial product instrument

| Задача | Статус |
|---|---|
| Отказаться от градиентных рамок, точек, контурных иконок и шаблонного segmented control | Зафиксировано |
| Сформулировать зачётку как editorial instrument: результат → очередь внимания → действие | Не начато |
| Сделать scope switch текстовой навигацией с нативным underline, без capsule UI | Не начато |
| Создать новый mockup с более сильной композицией и меньшим числом компонентов | Не начато |

# Mockup revision — native lower lesson focus surface

| Задача | Статус |
|---|---|
| Сохранить одобренную верхнюю часть зачётки | Зафиксировано |
| Переименовать вторую игру в `Собери по слогам` | Не начато |
| Заменить старомодный `уроки / ошибки` switch на native iOS segmented control | Не начато |
| Перестроить список уроков как строгий selectable list | Не начато |
| Передать обновлённый mockup для SwiftUI реализации | Не начато |

# Final mockup — minimal graphite grade sheet

| Решение | Статус |
|---|---|
| Сохранить graphite plane и строгий список без цветной заливки | Зафиксировано |
| Оставить gradient только в mastery number и активном selection state | Зафиксировано |
| Убрать радужные иконки/обводки и декоративный цветной шум | Зафиксировано |
| Сохранить segmented scope control и крупные lesson metrics | Зафиксировано |
| Сгенерировать финальный mockup перед SwiftUI redesign | Не начато |

# Mockup — зачётка курса, readable focus redesign

| Задача | Статус |
|---|---|
| Убрать случайные галочки и заменить их на единый selection control | Не начато |
| Увеличить и унифицировать числовую типографику lesson metrics | Не начато |
| Сделать «выбрать все / снять выбор» нативным segmented control | Не начато |
| Переосмыслить lesson error row как спокойный focus signal, а не Jira-like badge | Не начато |
| Создать обновлённый mobile mockup и зафиксировать SwiftUI implementation scope | Не начато |
