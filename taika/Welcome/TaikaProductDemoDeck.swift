...
enum TaikaProductDemoDeck {
    ...
    static let appIntro: [TaikaProductDemoScene] = [
        .init(
            id: "story_1",
            stepLabel: "1",
            title: "Одна фраза — твой первый шаг",
            subtitle: "Выбери живую фразу, услышь её, скажи сам — Taika подскажет следующий шаг.",
            stage: .phraseHero
        ),
        .init(
            id: "story_2",
            stepLabel: "2",
            title: "Taika слышит больше, чем слова",
            subtitle: "Разбор показывает произношение и тон, чтобы ты понял, что улучшить самому.",
            stage: .learnPipeline
        ),
        .init(
            id: "story_3",
            stepLabel: "3",
            title: "Одна фраза — несколько способов закрепить",
            subtitle: "Разминка, Match, слоги, Audio Recall и Speaker превращают попытку в навык.",
            stage: .reinforceWays
        ),
        .init(
            id: "story_4",
            stepLabel: "4",
            title: "Твой персональный кун кру рядом",
            subtitle: "Курсы, уроки, слова и фразы под твой ритм. Taika помогает учиться, а не отвечает за тебя.",
            stage: .speakerAI
        )
    ]
